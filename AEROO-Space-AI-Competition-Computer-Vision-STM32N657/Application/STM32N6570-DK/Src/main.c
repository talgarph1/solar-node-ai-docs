...
static void Ethernet_MPU_Config(void);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    USB_CDC_SendDetections(&pp_output, ts[1] - ts[0]);
    udp_sender_send_detections(&pp_output, ts[1] - ts[0]);
...
static void Ethernet_MPU_Config(void)
{
  ...
  extern uint8_t __eth_start__;
  extern uint8_t __eth_end__;
  uint32_t base = (uint32_t)&__eth_start__;
  uint32_t limit = (uint32_t)&__eth_end__;
  ...
  attr.Attributes = INNER_OUTER(MPU_DEVICE_NGNRNE);
...
static void Ethernet_init(void)
{
  ...
  IP4_ADDR(&remote, 192, 168, 10, 10);
  udp_sender_init(&remote, 5001);
}
...
static void SystemClock_Config(void)
{
  ...
  /* USB1 OTG HS clock */
  RCC_PeriphCLKInitStruct.PeriphClockSelection |= RCC_PERIPHCLK_USBPHY1 | RCC_PERIPHCLK_USBOTGHS1;
  RCC_PeriphCLKInitStruct.ICSelection[RCC_IC15].ClockSelection = RCC_ICCLKSOURCE_PLL4;
  RCC_PeriphCLKInitStruct.ICSelection[RCC_IC15].ClockDivider = 1;
  RCC_PeriphCLKInitStruct.UsbPhy1ClockSelection = RCC_USBPHY1REFCLKSOURCE_OTGPHY1;
  RCC_PeriphCLKInitStruct.UsbOtgHs1ClockSelection = RCC_USBOTGHS1CLKSOURCE_IC15;
